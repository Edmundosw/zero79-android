...
public class MainActivity extends Activity {
    ...
    private void injectPdfDownloadBridge(WebView view) {
        String js = "javascript:(function(){" +
                ...
                // Helper used when the WebView tries to navigate directly to a blob: URL.
                ...
                "AndroidPdfBridge.saveBase64(i>=0?x.substring(i+1):x,b.type||'application/pdf',n||'ZERO79.pdf');" +
                "};" +
                "fr.readAsDataURL(b);" +
                ...
                // Intercept anchor clicks generated by html2pdf and other PDF download code.
                "var originalClick=HTMLAnchorElement.prototype.click;" +
                "HTMLAnchorElement.prototype.click=function(){" +
                "var a=this,h=a.href||'';" +
                "if(h.indexOf('blob:')===0){" +
                "window.__zero79SaveBlob(h,a.download||'ZERO79.pdf');return;" +
                "}" +
                "return originalClick.call(a);" +
                "};" +

                // Intercept window.open(blob:...) used by some PDF implementations.
    ...
    private void saveBlobFromWebView(String blobUrl) {
        if (webView == null) {
        ...
        String encoded = Base64.encodeToString(
                blobUrl.getBytes(StandardCharsets.UTF_8),
                Base64.NO_WRAP
        );

        String js = "(function(){" +
                "var u=decodeURIComponent(escape(atob('" + encoded + "')));" +
                "if(window.__zero79SaveBlob){" +
                "window.__zero79SaveBlob(u,'ZERO79.pdf');" +
                "}else{" +
                "AndroidPdfBridge.error('Bridge do PDF não foi carregado.');" +
                "}" +
                "})()";
...
main.post(() -> Toast.makeText(context, "PDF salvo em Downloads: " + finalSafeName, Toast.LENGTH_LONG).show());
    ...
    private boolean handleUrl(Uri uri) {
        ...
        if (scheme.equals("blob")) {
            saveBlobFromWebView(uri.toString());
            return true;
        }
